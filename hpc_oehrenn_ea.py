...
def hamiltonian(x, y, parameters):
    ...
    # Avoided crossing: diagonal includes constant gap parameter c
    c = parameters.get('c', 0.05)

    # Create and return Hamiltonian
    return 0.5*parameters['s'] * np.array([
        [parameters['a']*(z + c), off_diag],
        [np.conjugate(off_diag), -(z + c)]
    ], dtype=complex)
...
def berry_phase(x, y, parameters, state='lower'):
    """
    Calculate the analytical Berry phase for elliptical avoided crossing.

    Lower state: pi - (1+a)*pi*(r+c) / sqrt((1+a)^2*(r+c)^2 + 4*r^2)
    ...
    # r = sqrt(x^2 + (1-e^2)*y^2), reduces to sqrt(x^2+y^2) when e=0
    r = np.sqrt(np.real(x)**2 + np.sqrt(1 - e**2) * np.real(y)**2)

    ap1 = 1 + a  # (1+a)
    denom = np.sqrt(ap1**2 * (r + c)**2 + 4 * r**2)

    if denom < 1e-10:
        return 0.0

    phase_lower = np.pi - ap1 * np.pi * (r + c) / denom
...
def _berry_curvature_ead(x, y, a_param, c):
    """
    Berry curvature for elliptical avoided crossing (e=0).
    Generalized from bcurvEAD.ipynb for arbitrary Hamiltonian parameter a and gap c.

    Intermediates derived from the eigenvalue structure of:
    H = 0.5*s * [[a*(r+c), x-iy], [x+iy, -(r+c)]]
    """
    r = np.sqrt(x**2 + y**2)
    if r < 1e-15:
        return 0.0
    ...
    # Half eigenvalue gap: a_nb = 0.5*sqrt((1+a)^2*(r+c)^2/4 + r^2)
    a_nb = 0.5 * np.sqrt(ap1**2 * (r + c)**2 / 4 + r**2)

    # Diagonal-gap vs full-gap difference
    cv = ap1 * (r + c) / 2 - 2 * a_nb
    ...
    # Combined factor
    e_t = 2 * (1 + cv2_r2)**1.5 * d_c

    # h = d(a_nb^2)/dy = y * ((1+a)^2*(r+c) + 4r) / (8r)
    h = y * (ap1**2 * (r + c) + 4 * r) / (8 * r)

    # f = dc_var/dx
    F_num = 4 * ap1 * a_nb - ap1**2 * (r + c) - 4 * r
    f = x * F_num / (8 * r * a_nb)
    ...
    # Berry curvature expression (from bcurvEAD.ipynb, generalized)
    bcl = (1j * cv / (2 * a_nb * b**2 * d_c**2 * e_t * r**3 * dc_c**2)) * (
        -2j * (
            d_c * h * r * (2j * b * cv**2 * d_c - 1j * e_t * r**2)
            + a_nb * b * cv * d_c**2 * ((-k + 1j*l) * r**3 + cv * (2*f*r - 2j*y))
            + a_nb * e_t * r**2 * (2*cv*r - d_c*f*r + 1j*d_c*y)
        )
        + (1j * b * e_t * r**2 * dc_c * (
            2j * cv**2 * h * r - 1j * d_c * h * k * r**3
            + a_nb * (
                cv * (k + 1j*l) * r**3
                + cv**2 * (-2*f*r - 2j*y)
                + d_c * r**2 * (-1j*f*l*r + 1j*k*y)
            )
        )) / (np.sqrt(1 + cv2_r2) * (cv**2 + r**2))
        + 2 * dc_c * (
            e_t * r**2 * (1j*a_nb*f*r + h*r - a_nb*y)
            + b * d_c**2 * (
                cv * f * (-2*h*r + 2*a_nb*y)
                + r**2 * (-h*k*r - 2*a_nb*f*l*r + a_nb*k*y)
            )
        )
    )
...
# Berry curvature calculation for elliptical avoided crossing
def berry_curvature(x, y, parameters):
    """
    Calculate Berry curvature for elliptical avoided crossing.
    Uses analytical expression from bcurvEAD.ipynb, generalized for parameter a.
    Assumes e=0.
    ...
    """
    try:
        a_param = parameters['a']
        c = parameters.get('c', 0.05)
        return _berry_curvature_ead(x, y, a_param, c)
